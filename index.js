...
async function listSightings() {
  ...
  return db.select().from(sightings).orderBy(desc(sightings.sightedAt), desc(sightings.createdAt));
}
async function createSighting(input) {
  ...
  const result = await db.insert(sightings).values(input);
  const insertedId = Number(result[0].insertId);
  const created = await db.select().from(sightings).where(eq(sightings.id, insertedId)).limit(1);
  if (!created[0]) {
    throw new Error("Sighting could not be retrieved after creation");
  }
  return created[0];
}
